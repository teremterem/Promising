...
**Configuration:** Three parameters control when Promises start executing. `everything_starts_soon_by_default` is a per-Promise local override for the global `EVERYTHING_STARTS_SOON_BY_DEFAULT`; `INHERIT` (the default) propagates the parent's resolved value, `GLOBAL_DEFAULT` always reads the live global, and a concrete `bool` fixes it locally. `start_soon` decides whether the Promise starts immediately upon creation (or at the nearest available event loop window) or defers until awaited; `NOT_SET` (the default) defers to the parent's `children_start_soon_by_default` if that is enforced (a concrete `bool`), otherwise falls back to `everything_starts_soon_by_default`; `INHERIT` copies the parent's own `start_soon` directly (falling back to `everything_starts_soon_by_default` if there is no parent). `children_start_soon_by_default`, when set to a concrete `bool`, enforces that value as the `start_soon` default for child Promises that leave `start_soon` as `NOT_SET`; `NOT_SET` (the default) means no enforcement; `INHERIT` copies the parent's `children_start_soon_by_default`. The global `EVERYTHING_STARTS_SOON_BY_DEFAULT` is `True` by default and can be overridden via `promising.EVERYTHING_STARTS_SOON_BY_DEFAULT = False`.
