...
- `Promise[T]` (`promising/promise.py`) - Extends `asyncio.Future` with hierarchical context management. Uses `ContextVar` (`Promise._current`) to track the currently active Promise. Key lifecycle: `__init__` → `_create_task()` (if `start_soon`) → `_fulfill()` (activates context, runs coro, waits for children, sets result) → `_finalize()` (resets context). Uses `AsyncioBackedConcurrentFuture` (`promising/utils.py`) for thread-safe bridging to `concurrent.futures.Future`. Configuration is handled directly on Promise via `start_soon`, `children_start_soon_by_default`, and `everything_starts_soon_by_default` parameters, which are either set to concrete boolean values or use sentinels (`INHERIT`, `NOT_SET`, `GLOBAL_DEFAULT`) to control inheritance from parent Promises.
