...
The base class for hierarchical context management. Manages parent-child relationships, namespacing (`namespace` parameter), configuration inheritance (`children_start_soon`, `start_soon_default`, `thread_pool`), child-waiting (`await_children` / `await_children_sync`), child inspection (`collect_active_children`), and trace/debugging (`get_trace`, `format_trace`, `print_trace`). Also provides `get_parent_promise()` to walk up past non-Promise contexts. Uses a `ContextVar` (`PromisingContext.__active_context`) to track the currently active context.

**Lifecycle and child tracking.** Each `PromisingContext` keeps an `_active_children: set[PromisingContext]` (a strong-ref set, not a `WeakSet`) protected by a `threading.Lock`. Children are added via `_register_children_threadsafe()` when they are constructed and removed via `_unregister_children_threadsafe()` once they are both *closed* and have no remaining active descendants. A context is "closed" when its `with` block has exited (`close_context_threadsafe()` runs in `__exit__`'s `finally`); for `PromisingFuture` subclasses, it is also closed when `set_result()` / `set_exception()` is called. Closed contexts that still have active descendants stay registered until those descendants drain — this is what `collect_active_children` traverses recursively. Attempting to register a child on an already-closed context raises `ContextAlreadyClosedError`; re-entering an already-closed context raises the same error.

This file also defines `PromisingFuture[T_co]` — a `PromisingContext` that is also an `asyncio.Future`. It is the new intermediate class between `PromisingContext` and `Promise`: anything that should appear as an awaitable child in the hierarchy must be a `PromisingFuture` (registering an `Awaitable` child that is not a `PromisingFuture` is rejected as a `TypeError`). `PromisingFuture` overrides `set_result()` / `set_exception()` to call `close_context_threadsafe()` before delegating to the asyncio Future, so the future and its context lifecycle stay in sync.
...
Extends `PromisingFuture[T_co | Promise[T_co]]` (which itself combines `PromisingContext` and `asyncio.Future`). Adds coroutine execution lifecycle on top of the hierarchy: `__init__` → `_ensure_task_scheduled()` (if `start_soon`) → `_fulfill()` (activates context, runs coro, sets result) → context restoration (resets `ContextVar` token). Also contains `PromiseBackedConcurrentFuture` for thread-safe bridging to `concurrent.futures.Future`. Both `__await__` and `PromiseBackedConcurrentFuture`'s blocking methods (`result()`, `exception()`) call `_ensure_task_scheduled()` before waiting, so deferred Promises (`start_soon=False`) are automatically started when consumed — no manual scheduling is needed.

A `Promise` constructed without an `awaitable` (i.e. with `prefilled_result` / `prefilled_exception`) passes `close_context_immediately=True` to `PromisingFuture.__init__`, so it is born already closed — there is no coroutine to run inside a `with self:` block, so nothing would otherwise close the context.

**Awaitable auto-wrapping:** When a Promise's result (set via `set_result()`) is an awaitable that is not already a `Promise`, it is automatically wrapped in a child `Promise`, propagating the parent's `loop`, `thread_pool`, `start_soon`, `children_start_soon`, and `start_soon_default` settings. This guarantees that `unpack_once()` and `unpack_once_sync()` always return either a concrete value or a `Promise` (never a plain awaitable).
...
**Unpacking semantics:** A `PromisingFunction` always returns a `Promise`, regardless of whether the underlying function returns a concrete value, a coroutine, or another `Promise`. From there, `await promise` (and its coroutine alias `promise.unpack_all()`) and `promise.unpack_all_sync()` (aliased as `promise.sync()`) always return a concrete value — they recursively unpack nested Promises until a non-Promise result is reached. Importantly, unpacking only traverses `Promise` instances specifically — it does not unpack arbitrary awaitables or `PromisingFuture` objects in general. `promise.unpack_once()` and `promise.unpack_once_sync()` unpack only one level, returning either a concrete value or another `Promise` (never a plain awaitable, thanks to auto-wrapping above). Notably, `unpack_once_sync()` returns the same dual-purpose `Promise` objects as `unpack_once()` — the caller can continue with `.sync()` in a sync context or switch to `await` in an async one. The async unpacking logic lives in `_AwaitablePromiseUnpacker`, a helper class used by both `__await__` (with `unpack_all=True`) and `unpack_once` (with `unpack_all=False`).

**Why `unpack_all()` exists:** `Promise` is an `asyncio.Future`, so utilities like `asyncio.wait_for`, `asyncio.gather`, and `asyncio.shield` detect it and wait on it directly, bypassing `__await__` and its recursive unpacking. `promise.unpack_all()` is a coroutine (not a Future), so passing it to those utilities forces them to wrap it in a Task that does invoke `__await__`, which restores the recursive unpacking behavior.
...
- **`protected_run()`** returns a **coroutine** (not a `Promise`), making it safe to pass to `asyncio.run()` — unlike calling the decorated function directly, which would construct a `Promise` (an `asyncio.Future` subclass) before the event loop exists and fail. Inside, the coroutine calls the decorated function, awaits the resulting `Promise`, and then — in a `finally` block, so regardless of success or failure — awaits its children (controlled by the `await_children` parameter, defaulting to `WHOLE_SUBTREE`). Used by `run()` internally.

### Sentinel Pattern (`promising/sentinels.py`)

`UNCHANGED`, `INHERIT`, `PROMISING_DEFAULT`, `ASYNCIO_DEFAULT`, and `WHOLE_SUBTREE` raise `SentinelUsageError` on boolean coercion to prevent misuse. `UNCHANGED` means "no call-time override — use the decorator-level value", `INHERIT` means "inherit from parent", `PROMISING_DEFAULT` means "read the current global setting directly", `ASYNCIO_DEFAULT` means "let the event loop use its own default executor", `WHOLE_SUBTREE` means "await all descendants recursively" (used as the default for `await_children` in `PromisingFunction.run()` and `protected_run()`).
