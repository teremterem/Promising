...
- **`protected_run()`** is the async counterpart of `run()` — an entrypoint for use when you already have a running event loop. It creates a `Promise`, awaits it, and then awaits its children (controlled by the `await_children` parameter, defaulting to `RECURSIVELY`).
