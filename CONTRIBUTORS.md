...
- **`run()`** is a convenience method that calls `asyncio.run()` on `protected_run()`, allowing the decorated function to be run from non-async code. By default, it awaits all children recursively after the function completes (`await_children=RECURSIVELY`).
- **`protected_run()`** is the async counterpart — it creates a `Promise`, awaits it, and then awaits its children (controlled by the `await_children` parameter, defaulting to `RECURSIVELY`).
