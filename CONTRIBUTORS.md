...
**Late parent registration.** `Promise.__init__` passes `register_with_parent=False` to `PromisingContext.__init__` and only calls `_register_with_parent()` at the very end of its own constructor, after the state machine has been seeded (including the prefilled `_FINISHED` / exception path). This guarantees that a Promise whose construction raises is never visible to its parent's child set, and that the prefilled-Promise case described above falls out naturally — by the time `_register_with_parent` runs, `done()` is already `True`, so the registration is skipped.
...
- `PromiseInvalidStateError(PromisingError, asyncio.InvalidStateError, concurrent.futures.InvalidStateError)` — base class for Promise state-related errors
  - `PromiseNotDoneError` — `Promise.result()` / `Promise.exception()` was called before the Promise was done
  - `PromiseNotUnpackedError` — `Promise.intermediate_promise()` was called before the first unpacking step
...
- `_format_last_stack` — applied to the actual exception's traceback; collapses runs of `promising/promise.py` frames in the middle of the call stack while preserving frames from the rest of the package (e.g. framework input-validation errors) so framework-level bugs remain visible.

If any rendering step raises, the hooks fall back to the previously installed `sys.excepthook` / `threading.excepthook` and print a `FAILED TO PRINT PROMISING TRACE` warning. `examples/agents_with_errors.py` exercises all of the above (including the `__cause__` / `__context__` chain) and is the recommended manual smoke test for changes to this file.
