...
***TODO [P1]*** *This document was generated by AI - REVIEW and "HUMANIZE"*
...
**Unpacking semantics:** `await promise` and `promise.sync()` fully unpack nested awaitables — if the coroutine's result is itself awaitable, it is recursively resolved until a concrete, non-awaitable value is reached. These two are guaranteed to always return a concrete value. `promise.unpack_once()` and `promise.unpack_once_sync()` resolve only one level, returning either a concrete value or a `Promise` (never a plain awaitable, thanks to auto-wrapping above). The async unpacking logic lives in `_AwaitablePromiseUnpacker`, a helper class used by both `__await__` (with `unpack_all=True`) and `unpack_once` (with `unpack_all=False`).
