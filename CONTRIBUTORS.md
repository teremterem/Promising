...
**Exception breadcrumbs.** `try_to_link_exception` attaches the `PromisingContext` to an exception as `__promising_context__`, only at the deepest level (skips if already set). Primary attribution happens in `PromisingContext.__exit__`; `_set_exception_from_loop` and `_force_internal_error_finish_from_loop` also call it as a safety net for paths that don't pass through `__exit__`. The `sys.excepthook` / `threading.excepthook` overrides in `promising/errors.py` consume this attribute to print promising traces alongside the standard traceback.
