...
**Sync and thread-safe consumption.** `sync()` and `unpack_once_sync()` dispatch onto the Promise's own event loop via `asyncio.run_coroutine_threadsafe` and block the calling thread on the resulting `concurrent.futures.Future`. Both refuse to run on the Promise's loop thread (`_assert_no_sync_usage_deadlock` → `SyncUsageError`). `cancel()` is similarly thread-safe: when called from outside the loop, it dispatches `_cancel_from_loop` via `call_soon_threadsafe` and blocks until the cancellation has run.
