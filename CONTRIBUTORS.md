...
- **`protected_run()`** is an async convenience that creates a `Promise`, awaits it, and then — in a `finally` block, so regardless of success or failure — awaits its children (controlled by the `await_children` parameter, defaulting to `RECURSIVELY`). Used by `run()` internally, but can also be called directly when you already have a running event loop.
