...
| `promising.PromisingFunction.protected_run(*args, **kwargs)` | Async counterpart of `run()` — an entrypoint for use when you already have a running event loop (e.g. inside an `async def main()`). Calls the decorated function, awaits the resulting `Promise`, and then awaits its children (controlled by `await_children`, which defaults to `RECURSIVELY`). Accepts the same configuration overrides as `__call__`, plus `await_children`. |
