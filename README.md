...
| `promising.PromisingFunction.run(*args, **kwargs)` | Convenience method that calls `asyncio.run()` on `protected_run()`. Runs the decorated function from non-async code, awaits the result, and by default awaits all children recursively (`await_children=RECURSIVELY`). Accepts the same configuration overrides as `__call__`, plus `await_children`. |
| `promising.PromisingFunction.protected_run(*args, **kwargs)` | Async method that calls the decorated function, awaits the resulting `Promise`, and then awaits its children (controlled by `await_children`, which defaults to `RECURSIVELY`). Accepts the same configuration overrides as `__call__`, plus `await_children`. |
