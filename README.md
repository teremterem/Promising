...
| `promising.PromisingFunction.protected_run(*args, **kwargs)` | Returns a **coroutine** (not a `Promise`), making it safe to pass to `asyncio.run()` — unlike calling the decorated function directly, which would construct a `Promise` (an `asyncio.Future` subclass) before the event loop exists and fail. Inside, the coroutine calls the decorated function, awaits the resulting `Promise`, and awaits its children (controlled by `await_children`, which defaults to `WHOLE_SUBTREE`). Used by `run()` internally. Accepts the same configuration overrides as `__call__` (as well as all the parameters of the underlying decorated function), plus `await_children`. |
...
| `ctx.await_children(whole_subtree=True, fully_unpack_promises=True)` | Async — wait for child contexts to finish. With `fully_unpack_promises=False`, Promise children are only unpacked one level (via `unpack_once()`) instead of being fully awaited. |
| `ctx.await_children_sync(whole_subtree=True, fully_unpack_promises=True, timeout=None)` | Sync — block until child contexts finish. With `fully_unpack_promises=False`, Promise children are only unpacked one level instead of being fully awaited. |
...
| `promising.await_children(whole_subtree=True, fully_unpack_promises=True)` | Wait for all children of the current context. With `fully_unpack_promises=False`, Promise children are only unpacked one level. |
| `promising.await_children_sync(whole_subtree=True, fully_unpack_promises=True, timeout=None)` | Sync counterpart — block until children finish. With `fully_unpack_promises=False`, Promise children are only unpacked one level. |
