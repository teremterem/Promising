...
class Promise(Future, Generic[T_co]):
    ...
    async def _afulfill(self) -> None:
        ...
        """
        if self._coro is None:
            raise RuntimeError(f"Cannot fulfill a Promise that has no coroutine: {self.get_name()}")
    ...
    async def await_for_children(self) -> None:
        ...
        This method waits for all pending child promises that are configured to make their parent wait.
        Any exceptions raised by the children are captured but not re-raised.
        """
        promises_to_await = [
