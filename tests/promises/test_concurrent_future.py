...
async def test_from_threads(
    ...
    timeouts = [0.4, 0.4, 0.1]  # Third thread times out earlier

    def thread_function(index, timeout):
        nonlocal results
        try:
    ...
    threads = [threading.Thread(target=thread_function, args=(i, timeouts[i])) for i in range(3)]
