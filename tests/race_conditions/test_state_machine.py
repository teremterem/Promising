...
async def test_state_monotonic_under_concurrent_observers() -> None:
    ...
    stop.set()
    for t in threads:
        await asyncio.get_running_loop().run_in_executor(None, t.join, timeout=2.0)
...
async def test_repeated_set_calls_do_not_re_advance_terminal_state() -> None:
    ...
    promise._set_exception_from_loop(asyncio.CancelledError("late"))
    ...
    promise._set_exception_from_loop(RuntimeError("bogus"))
    ...
    promise._set_result_from_loop("override")
    assert promise._state is _FINISHED
    promise._set_intermediate_promise_from_loop(Promise(prefilled_result=99))
    assert promise._state is _FINISHED
