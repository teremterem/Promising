...
async def test_await_children_whole_subtree(*, whole_subtree: bool) -> None:
    ...
    With True: await_children_sync(whole_subtree=True) is called on the
    root, so every level completes before the root resolves.
    With False: await_children_sync(whole_subtree=False) only waits for
...
async def test_await_children_on_bare_context_whole_subtree(*, whole_subtree: bool) -> None:
    ...
    with promising.context() as ctx:
        ...
        await loop.run_in_executor(None, lambda: ctx.await_children_sync(whole_subtree=whole_subtree))
...
async def test_await_children_direct_only_but_fully_unpack_promises(
    *,
    sleep_in_root: bool,
    fully_unpack_promises: bool | None,
) -> None:
    """
    With ``whole_subtree=False``, ``await_children_sync`` waits only for direct
    children.  When ``fully_unpack_promises`` is True (or default), the child's
    ...
    def root_func() -> str:
        ...
        #  ensure it has effect ?
        kwargs = {}
        if fully_unpack_promises is not None:  # We use None to test the default
            kwargs["fully_unpack_promises"] = fully_unpack_promises
    ...
    if sleep_in_root:
        if fully_unpack_promises is False:
            assert execution_order == [
                "child_done",
                "root_coro_done",
            ]
    ...
    elif fully_unpack_promises is False:
        assert execution_order == [
            "root_coro_done",
            "child_done",
        ]
